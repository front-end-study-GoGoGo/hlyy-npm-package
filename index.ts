 ...
 * @returns 
 */
export const transferFloat = (num: any, toFixNum = 2, roundingOff = true): number => {
  if (!roundingOff) {
 ...
 */
export const areDataReckon = (type: string) => {
  ...
  //  本日
  if (type === '本日') {
    let todayDay = moment((moment().unix()) * 1000).format('YYYY-MM-DD');
    myData = [
  ...
  //  上日
  if (type === '上日') {
    let lastDay = moment((moment().unix() - 24 * 60 * 60) * 1000).format('YYYY-MM-DD');
    myData = [
      moment(lastDay, 'YYYY-MM-DD'),
      moment(lastDay, 'YYYY-MM-DD')
    ];
  }
  // 上周 
  ...
  // 上季度
    ...
    let today = moment((moment().unix()) * 1000).format('YYYY-MM-DD');
    let lastDay = moment((moment().unix()) * 1000).format('YYYY-MM-DD');
    myData = [
 ...
 * @returns 返回清洗完的对象
 */
export const areRinseObj = (obj: any, timeFormatString: boolean = false, startTime?: string, endTime?: string) => {
  let newObj = {};
  // 清洗对象同时对时间格式进行转换
  if (timeFormatString) {
    for (let k in obj) {
